...
/**
 * 置顶：一枚**实心**图钉（宽头 + 锥身 + 宽箍 + 针），用于标题栏「窗口置顶」按钮.
 * 与最小化/最大化同属窗口控制图标，故与它们相邻定义。
 *
 * 为什么用实心而非线性（2026-09 改版）：线性图钉在 12–14px 下只剩一圈细线，
 * 视觉权重明显低于旁边的关闭「×」，用户反馈「不够明显」。改实心后同尺寸可读性显著提升，
 * 与文件头「实心图标用于需要高辨识度的小尺寸场景」的定位一致。
 *
 * 形状按**小尺寸可读**设计：头 / 箍两根横杠提供宽度，中间的锥身留出腰部，
 * 使整体不至于糊成一块。实测 Material 的 push_pin（头颈箍粘连）在 14px 下就是一块黑方块，
 * 无箍的「宽头 + 长锥」会读成漏斗，故都不采用。
 *
...
export function PinIcon(props: IconProps) {
  return (
    <svg {...solidBase(props)}>
      {/* 头：宽横杠 */}
      <rect x="5" y="2.5" width="14" height="3.5" rx="1.2" />
      {/* 锥身：上宽下窄，在头与箍之间留出腰部 */}
      <path d="M8.5 6h7l-1.2 5.5h-4.6z" />
      {/* 箍：第二根横杠 */}
      <rect x="5.5" y="11.5" width="13" height="3" rx="1.2" />
      {/* 针：向下收细，末端圆角 */}
      <path d="M10.6 14.5h2.8l-.6 6.6a.8.8 0 0 1-1.6 0z" />
    </svg>
  )
}
